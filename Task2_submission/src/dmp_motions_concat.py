...
def learn_dmp(bag_path, start_pos, end_pos):
    # Example usage:
    ...
    new_start[:3] += start_pos
    new_goal[:3] += end_pos  # Modify position
...
def exec_dmp(bag_path):
    pass

# -------------------------------------- MAIN --------------------------------------# 
if __name__ == "__main__":
    bag_paths = ['/root/catkin_ws/recordings/home1.bag']
    start_poss = [np.array([-0.1,0.,0.02])]
    end_poss = [np.array([0.0, 0.0, 0.02])]

    # Later, you can reload and publish it
    joint_traj, time_stamps = learn_dmp(bag_paths[0], start_poss[0], end_poss[0])
    #load_trajectory_data("/root/catkin_ws/recordings/interpolated_traj.pkl")
