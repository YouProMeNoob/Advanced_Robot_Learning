...
def get_xyz_from_world_to_one():
    ...
    listener.waitForTransform('/world', '/one', rospy.Time(0), rospy.Duration(10.0))
    
    # Get the latest available transform
    trans, _ = listener.lookupTransform('/world', '/one', rospy.Time(0))
    
    return trans 


def get_xyz_from_world_to_two():
    if not rospy.core.is_initialized():
        rospy.init_node('tf_xyz_fetcher', anonymous=True)
    ...
    # Wait until the transform becomes available
    listener.waitForTransform('/world', '/two', rospy.Time(0), rospy.Duration(10.0))
    
    # Get the latest available transform
    trans, _ = listener.lookupTransform('/world', '/two', rospy.Time(0))
...
def get_xyz_from_world_to_three():
    ...
    # Wait until the transform becomes available
    listener.waitForTransform('/world', '/three', rospy.Time(0), rospy.Duration(10.0))
    
    # Get the latest available transform
    trans, _ = listener.lookupTransform('/world', '/three', rospy.Time(0))
    
    return trans 


def get_xyz_from_world_to_four():
    if not rospy.core.is_initialized():
        rospy.init_node('tf_xyz_fetcher', anonymous=True)
    ...
    # Wait until the transform becomes available
    listener.waitForTransform('/world', '/four', rospy.Time(0), rospy.Duration(10.0))
    
    # Get the latest available transform
    trans, _ = listener.lookupTransform('/world', '/four', rospy.Time(0))
...
if __name__ == "__main__":
    ...
    bag_path = '/root/catkin_ws/recordings/pick6.bag'
    ...
    print(f"gripper_traj = {gripper_traj}")
    gripper_traj += 0.0025

    # Save the learned DMP if needed
    dmp_path = '/root/catkin_ws/recordings/learned_pick_motion_6.pkl'
    ...
    new_goal[:3] = np.array(trans_three)  # Modify position
    new_goal[:3] += np.array([-0.01,0.02,-0.015]) 
...
########################Second
    urdf_path = '/root/catkin_ws/src/open_manipulator_friends/open_manipulator_6dof_description/urdf/open_manipulator_6dof.urdf'
    mesh_path = '/root/catkin_ws/src/open_manipulator_friends/open_manipulator_6dof_description/meshes'
    bag_path = '/root/catkin_ws/recordings/lift6.bag'
    ...
    # Save the learned DMP if needed
    dmp_path = '/root/catkin_ws/recordings/learned_lift_motion_6.pkl'
    dmp_gen.save_dmp(dmp_path)
    #dmp_gen.load_dmp(dmp_path)
    ...
    new_start[:3] += np.array([-0.01,0.02,0.01])
...
###########################Third
    urdf_path = '/root/catkin_ws/src/open_manipulator_friends/open_manipulator_6dof_description/urdf/open_manipulator_6dof.urdf'
    mesh_path = '/root/catkin_ws/src/open_manipulator_friends/open_manipulator_6dof_description/meshes'
    bag_path = '/root/catkin_ws/recordings/place6.bag'
    ...
    # Save the learned DMP if needed
    dmp_path = '/root/catkin_ws/recordings/learned_place_motion_6.pkl'
    dmp_gen.save_dmp(dmp_path)
    #dmp_gen.load_dmp(dmp_path)
    ...
    trans_four=get_xyz_from_world_to_four()
    print(trans_four)
    
    new_start[:3] += np.array([0.0, 0.0, 0.0])
    new_goal[:3] = np.array(trans_four)  # Modify position
    new_goal[:3] += np.array([0.00,0.0,0.04]) 
...
################################# Home
    urdf_path = '/root/catkin_ws/src/open_manipulator_friends/open_manipulator_6dof_description/urdf/open_manipulator_6dof.urdf'
    mesh_path = '/root/catkin_ws/src/open_manipulator_friends/open_manipulator_6dof_description/meshes'
    bag_path = '/root/catkin_ws/recordings/home6.bag'
    ...
    # Save the learned DMP if needed
    dmp_path = '/root/catkin_ws/recordings/learned_home_motion_6.pkl'
    dmp_gen.save_dmp(dmp_path)
    #dmp_gen.load_dmp(dmp_path)
    ...
    new_start[:3] += np.array([0.0, 0.0, 0.04])
    new_goal[:3] += np.array([0.0, 0.0, 0.0])  # Modify position
